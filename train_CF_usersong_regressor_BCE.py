...
from CF_dataloader import CFDataloader
from utils.plot_save_png import regression_img_save
cudnn.benchmark = True

parser = argparse.ArgumentParser(description="cf_model")
parser.add_argument("-s","--save_path",type=str, default="./save/exp_cf_mlp_BCE/")
...
scheduler = StepLR(optim, step_size=50, gamma=0.6)  
...
#%% Train/test methods
def Train():
    mtrain_loader = CFDataloader(mtrain_mode=True, normalization_factor=.01,
                                 binaray_mode=False, batch_size=TR_BATCH_SZ, shuffle=True)
    ...
    for epoch in trange(0, EPOCHS, desc='epochs', position=0, ascii=True):
        ...
        for it in range(len(tr_iter)):
            ...
            loss = loss_L1(input=y_hat, target=y) + loss_BCE(input=y_hat, target=y)
            model.zero_grad()
            loss.backward()            
            optim.step()
        ...
        tqdm.write("tr_epoch:{0:}  tr_loss:{1:.6f}  val_loss:{2:.6f}".format(epoch, 
...
def Validate():
    mval_loader = CFDataloader(mtrain_mode=False, normalization_factor=.01, batch_size=TS_BATCH_SZ, shuffle=False)
